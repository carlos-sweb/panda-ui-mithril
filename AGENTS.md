...
```
src/components/    70 components, one folder each: index.js + index.d.ts
                    (Button and Alert are .jsx — JSX with Mithril's factory;
                     the other 68 are .js using direct m() hyperscript)
src/recipes/       66 Panda recipes in TypeScript (cva/sva) — single source of
...
src/preset.ts      pumPreset: semantic tokens, dark-mode conditions, keyframes,
                    globalCss, globalVars (consumed by the consumer's Panda)
...
playground/        Demo site (72 pages) — see Playground Rules
scripts/           Bun build/verify scripts
dist-playground/   Build output of `bun run build` — never committed
```
...
5. **Use the spacing/fontSizes token scale instead of rem literals.** Spacing
   ...
   `2xs`–`9xl` for fontSizes), with only three extra spacing keys in
   `panda.config.ts`: `'128'` (32rem), `'192'` (48rem), `'320'` (80rem).
...
- Stage everything except the build artifact:
  `git add -A -- . ':(exclude)dist-playground'`
- Conventional commit messages: `feat(...)`, `fix(...)`, `docs(...)`,
  `refactor(...)`. Scope = component name when applicable
  (`feat(dropdown): ...`).
- `dist-playground/` and `node_modules/` are never committed.
...
- Never commit `node_modules/` or `dist-playground/`
